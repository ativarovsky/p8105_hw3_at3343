...
want to pivot the data so that minutes are observations and activity is
the data. We do this using pivot\_longer. We then rename activity to
minute and round all the activity values to two digits and convert
activity to a numeric vector. Finally, we add a variable to indicate
whether the line item corresponds to a weekend observation (day =
Saturday or Sunday) or a weekday observation (day = Monday- Friday).
